...
import json
from multiwoz.Delexicalize import normalize
import sqlite3
import os
import random
...
class MultiWozEvaluator(BaseEvaluator):
    def __init__(self, data_name, delex_path='data/multi-woz/delex.json'):
        self.data_name = data_name
    ...
    def _evaluateGeneratedDialogue(self, dialog, goal, realDialogue, real_requestables, soft_acc=False):
                ...
                for requestable in requestables:
                    if requestable == 'reference':
                        ...
                        if domain + '_' + requestable + ']' in sent_t:
                            ...
                            # print(domain + '_' + requestable, sent_t)
        ...
        if success==0 and False: # change to True if print errors for debug
            print('match: ', match)
    ...
    def _parse_entities(self, tokens):
        entities = []
        for t in [str(s) for s in tokens]:
            if '[' in t and ']' in t:
                entities.append(t)
        return entities

    def evaluate_match_success(self, dialogues, mode='Valid'):
        ...
        for filename, dial in dialogues.items():
            if mode == 'rollout':
                ...
                success, match, stats = self._evaluateGeneratedDialogue(dial, goal, data, requestables, soft_acc=False)
        ...
        # report='success: '+str(successes)+' '
        # report += 'Corpus Matches : {:2.2f}%'.format((matches / float(total) * 100)) + " "
        # report += 'Corpus Success : {:2.2f}%'.format((successes / float(total) * 100)) + " "
        # report += 'Total number of dialogues: %s ' % total

        return successes/float(total)*100, matches/float(total)*100

    def evaluate_bleu_prf(self, dialogues):
        delex_dialogues = self.delex_dialogues
        tokenize = lambda x: x.split()
        refs, hyps = [], []
        tp, fp, fn = 0, 0, 0
        for filename, dial in dialogues.items():
            real_diag = delex_dialogues[filename]
    ...
    def summarize_report(self, dialogues, mode='Valid', pt_values=True):
        ...
        SCORE2 = 0.5 * MATCHES + 0.5 * SUCCESS + 1*BLEU
        ...
        return BLEU, MATCHES, SUCCESS, SCORE, P, R, F1, SCORE2
